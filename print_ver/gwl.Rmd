...
that have been monitored for ten years or more and were active as of 2009.
...
by natural resource region.

\vspace{30pt}

```{r bc-bar, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 8, fig.height = 6, fig.align = 'center'}
plot(bc_bar_chart)
```

\newpage

```{r region, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 7, fig.height = 8, fig.align = 'center'}
...
```{r overview_map, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results = 'asis', fig.height = 8, fig.align = 'center'}
plot(summary_map)
```
...
-   Visit the [Monitoring Aquifers: Provincial Observation Well Network homepage](http://www2.gov.bc.ca/gov/content?id=B03D0994BB5C4F98B6F7D4FD8610C836) for more information on groundwater monitoring observation wells.
...
## Appendix 1: Detailed Maps & Graphs for Groundwater Observation Wells Included in the Analysis
...
for (reg in sort(unique(attr_wells$REGION_NAME))) {
  ...
  for (well in regdata$Well_Num) {
    ...
    mapplot <- filter(well_plots, type == "map", well_num == well) %>% pull(plot)
    monthplot <- filter(well_plots, type == "month", well_num == well) %>% pull(plot)
    areaplot <- filter(well_plots, type == "area", well_num == well) %>% pull(plot)
...
- **Stable**: the trend was _not_ significantly different from zero **or** the trend was significantly different from zero with an increasing or decreasing trend of _less than_ 3 cm per year.
