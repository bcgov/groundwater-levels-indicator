...
load('../groundwater-levels-indicator/tmp/results_viz.RData')
#load('../tmp/well_plots.RData')
load('../groundwater-levels-indicator/tmp/figures.RData')
load('../groundwater-levels-indicator/tmp/well_data_attributes.RData')

attr_wells <- results_viz

attr_table <- results_for_table
...
in groundwater levels recorded at `r nrow(attr_wells)` observation wells that have been monitored for ten years or more and that were active as of `r active_years`.

-  Groundwater level trends are summarized using four long-term trend categories: Increasing (increasing by more than 3 cm/year), Stable, Moderate Rate of Decline (decreasing by 3 to 10 cm/year), and Large Rate of Decline (decreasing by more than 10 cm/year).
...
\definecolor{increasing}{HTML}{`r substr("#2c7bb6", 2, 7)`}
\definecolor{stable}{HTML}{`r substr("white", 2, 7)`}
\definecolor{moderate}{HTML}{`r substr("#fdae61", 2, 7)`}
\definecolor{large}{HTML}{`r substr("#d7191c", 2, 7)`}
\definecolor{missing}{HTML}{`r substr("grey3", 2, 7)`}

### Trend Analysis

Trend analysis was conducted using monthly groundwater levels from each
groundwater observation well that had ten or more years of approved or validated data, fewer
than 25% of monthly observations missing, and was active as of January 1,
`r active_years`. This resulted in `r nrow(attr_wells)` wells with suitable data records for analysis. 
...
substituted into the original time series where there were missing data.
...
-   **Stable** the trend was not significantly different from zero or the trend was significantly different from zero with an increasing or declining trend of less than 3 cm per year.
-   **Increasing:** the trend was significantly different from zero with an increasing trend of more than 3 cm per year.
...
### Appendix A: Summary of results of trend analysis of groundwater levels

Note that only wells with enough data for trend analysis are included. Results are presented for all data, 10 years, and 20 years, where groundwater level trend analysis was conducted on each respective time frame for comparison. Trend significance is indicated with asterisks, where one asterisk indicates a p-value of <0.01 (high correlation) and two asterisks indicate a p-value of 0.01-0.05 (moderate correlation). Table rows are coloured by the Results-All trend category. 

\small
```{r table, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results = 'asis'}

out.table <- attr_table[,c("REGION_NAME", "Well_Num", "start_date", "last_date",  
                            "Results_All", "Results_10yrs", "Results_20yrs")]
...
out.table <- out.table %>%
  select(REGION_NAME, Well_Num, date_range, Results_All, Results_10yrs, Results_20yrs)
...
HL_col <-  case_when(out.table$Results_All %like% "Large Rate of Decline%" ~  "\\rowcolor{large}",
                     out.table$Results_All %like% "Moderate Rate of Decline%" ~ "\\rowcolor{moderate}",
                      out.table$Results_All %like% "Increasing%" ~ "\\rowcolor{increasing}",
                      out.table$Results_All %like% "Stable%" ~ "\\rowcolor{stable}",
                      out.table$Results_All %like% "Too%" ~ "\\rowcolor{missing}")
...
names(out.table) <- c("Region", "Well", "Date Range", 
                      "Results-All Data", "Results-10 Years", "Results-20 Years")

out.table$Well <- round(as.numeric(out.table$Well), 0)

x.table <- xtable(out.table, digits = c(0,0,0,0,0,0,0), align = c('p{0.5in}', 'l', rep('l', 2), 'p{0.9in}', 'p{0.9in}','p{0.9in}'))
...
<!-- ## Appendix B: Detailed Maps & Graphs for Groundwater Observation Wells Included in the Analysis -->
