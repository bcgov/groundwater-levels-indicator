...
-   Of the `r nrow(attr_wells)` observation wells examined,
    `r round(sum(state.summary[c("Stable and/or Non-significant","Increasing"),"prop"]*100))`%
    have water levels that are stable, non-significant trends, or
    increasing (with `r state.summary["Increasing","n"]` wells showing
    increasing trends).
    `r round(state.summary["Moderate Rate of Decline","prop"]*100)`% of
    wells show a moderate rate of decline in water levels, and
    `r round(state.summary["Large Rate of Decline","prop"]*100)`% show a
...
-   The number of observation wells in each [Natural Resource
    ...
    across B.C. varies. The highest number of observation wells is in
    the West Coast Region, with 62 wells, and lowest number is in the
    Skeena and Omineca regions (2 wells each).
...
-   The location and status for each observation well can be seen in the
    map below. Opaque symbols represent stations with a statistically
    significant trend (colour represents direction and magnitude), while
    semi-transparent symbols represent locations with non-significant
    results. The observed long-term trends were categorized using a
    combination of statistical and environmental significance criteria.
    Based on both the significance and magnitude of the calculated
    trend, trends in water levels in wells were classified as exhibiting
    the following categories:
    -   **Stable or Increasing:** the trend was not significantly
        different from zero or the trend was significantly different
        from zero with an increasing trend or the trend was
        significantly different from zero with a declining trend of less
        than 3 cm per year.
    -   **Moderate Rate of Decline:** the trend was significantly
        different from zero with a declining trend of between 3 and 10
        cm per year.
...
had ten or more years of data, fewer than 25% of monthly observations
missing, and was active as of January 1, 2008. This resulted in
...
truncated to remove the missing values. Remaining missing values were
interpolated by first fitting a 12-period (i.e., monthly) basic
...
series; the predicted values were substituted into the orignal time
...
(Yue *et al*. 2002). In this method, the slope of each time series is
estimated using the Theil-Sen approach (Sen, 1968). If the estimated
slope is different from zero, then the data are detrended by the slope
and the AR(1) 1s calculated for the detrended time series. The residuals
and the trend are combined and then tested for significance using the
Mann-Kendall trend test. These methods were implemented using the R
package ‘zyp’ (Bronaugh *et al*, 2009).
...
-   **Stable or Increasing:** the trend was not significantly different
    from zero or the trend was significantly different from zero with an
    increasing trend or the trend was significantly different from zero
    with a declining trend of less than 3 cm per year.
-   **Moderate Rate of Decline:** the trend was significantly different
    from zero with a declining trend of between 3 and 10 cm per year.
-   **Large Rate of Decline:** the slope was significantly different
    from zero with a declining trend of more than 10 cm per year.
...
-   Bronaugh, D., Werner, A., & Bronaugh, M. D. (2009). Package ‘zyp’.
    CRAN Repository.
...
```{r well_plots, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results = 'asis', dev = "pdf"}
