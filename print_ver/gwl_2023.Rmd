...
-   Wells that have been monitored for \> 10 years, have fewer than 15% missing observations, and were active as of
    2013 are analyzed in this indicator. There are `r nrow(results_viz)`
    observation wells within the network that met the necessary screening criteria (i.e. sufficient data) and were assessed for changes
...
- The `r nrow(results_viz_10)` observation wells assessed are located in `r nrow(by_aquifer)` aquifers. The results of 10 year trend analysis for observation wells were grouped by aquifer. The results are grouped by aquifer as some aquifers have multiple wells with the same trending results which may skew the provincial and regional summary. 
...
-   Of the `r nrow(by_aquifer)` aquifers examined,
    ...
    trend analysis are included in the following figures. There are `r round(input_summary |> filter(state_figure == "Mixed") |> extract("prop"))`% of aquifers that have observation wells with different trend results
...
had ten or more complete years of data (data from each month of the year), had fewer than 15% missing observations, and were active as of January 1, 2013. This resulted in
`r nrow(attr_wells)` wells with suitable data records for analysis.For each trend option selected, the amount of data missing was calculated separately which led to several cases where there was enough data for one time period but not another (for example, a 10-year trend was calculated for a well but there was too much missing data to calculate a trend for the entire time series). 
...
values for all twelve months. Monthly trend analyses were conducted on mean groundwater levels found in a particular month over the time series. After the time series were constructed, each was inspected for consecutive missing values near the beginning or
...
Remaining missing values were interpolated by first fitting a 12-period (i.e., monthly) basic structural time series model (BSM) to the data (Harvey, 1990).
...
Three plots were included for each groundwater observation well that met the data requirements for data analysis. The plots included for each well include 10-year trends, all-data trends, and a monthly hydrograph. 

The 10-year and all-data plots show the history of the groundwater level over the relevant time period.  The blue line shows measured groundwater depth over time. The light grey dots
...
The graph at the bottom of the page shows monthly groundwater levels
...
```{r well_plots, echo=FALSE, error=FALSE, fig.height=6, message=FALSE, warning=FALSE, results='asis', eval=TRUE}
...
for (reg in sort(unique(well_plots$region_name))) {
  ...
  for (well in pdf_wells) {
    ...
    areaplot_10 <- filter(well_plots_10, Well_Num == well) %>% pull(area_plot)
    areaplot <- filter(well_plots, Well_Num == well) %>% pull(area_plot)
    
    plot(well_plots$area_plot[[well]])
    ...
    # grid.arrange(monthplot[[1]], areaplot_10[[1]], areaplot[[1]],
    #              nrow=3)
      ...
      #mapplot + 
                 ...
                 #                   plot.margin = unit(c(0.5,0.5,0.5,0.01),"cm"),
                 #                   legend.spacing.y = unit(-.4, 'cm')),
                  # areaplot_10[[1]] + theme(axis.text = element_text(size = 8),
                  #                 axis.title.x = element_blank(),
                  #                 axis.title.y = element_text(size = 8,
                  #                                             hjust = 0.5,
                  #                                             vjust = 1,
