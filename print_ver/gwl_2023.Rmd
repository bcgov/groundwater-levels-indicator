...
Groundwater provides drinking water to 1 in 4 British Columbians. Hundreds of groundwater aquifers provide water for industries, municipalities, farms, and rural homeowners. Aquifers are geological formations of permeable rock, gravel, or sand containing or conducting groundwater. British Columbia  operates a provincial [observation well network of over 200 wells](https://www2.gov.bc.ca/gov/content?id=B03D0994BB5C4F98B6F7D4FD8610C836). This network was established in 1961 to track groundwater availability in areas of high human use.

-   Observation wells are not used for domestic or commercial use. Rather, observation wells provide information on groundwater levels over time.
...
weather and climate patterns (Gurdak *et al*, 2009). [Aquifers](https://www2.gov.bc.ca/gov/content/environment/air-land-water/water/groundwater-wells-aquifers/understanding-aquifers) are geological formations of permeable rock, gravel, or sand containing or conducting groundwater.

-  This indicator analyses long-term trends in groundwater levels recorded at `r nrow(attr_wells)` observation wells. The analyses includes observation wells that were active as of 2013 and tracked for 10 years or more.

-  Groundwater level trends are summarised using three long-term trend categories: Stable (less than 3 cm/year or insignificant trend) or Increasing (3 to 5 cm/year), Moderate Rate of Decline (3 to 10 cm/year), and Large Rate of Decline (more than 10 cm/year).
...
-   Of the `r nrow(attr_wells)` observation wells examined, `r round(sum(state.summary[c("Stable","Increasing"),"prop"]*100))`% have water levels that are stable or increasing (with `r state.summary["Increasing","n"]` wells showing increasing trends). `r round(state.summary["Moderate Rate of Decline","prop"]*100)`% of wells show a moderate rate of decline in water levels, and `r round(state.summary["Large Rate of Decline","prop"]*100)`% show a large rate of decline. Note that only wells with enough data for trend analysis are included in the following figures.
...
- The number of observation wells in each [Natural Resource Regions](https://www2.gov.bc.ca/gov/content/industry/forestry/managing-our-forest-resources/ministry-of-forests-lands-and-natural-resource-operations-region-district-contacts) across B.C. varies. The highest number of observation wells is in the West Coast Region, with 62 wells, and lowest number is in the Skeena and Omineca regions (2 wells each).  
...
- The location and status for each observation well can be seen in the map below. Opaque symbols represent stations with a signficant trend (colour represents direction and magnitude), while semi-transparent symbols represent locations with non-signficant results. The observed long-term trends were categorized using a combination of statistical and environmental significance criteria. Based on both the significance and magnitude of the calculated trend, trends in water levels in wells were classified as exhibiting the following categories:
...
# summary map
# print(leaflet_map)
# map = image_crop(leaflet_map, "2000x1500+100")
plot(leaflet_map)
```
...
Groundwater levels are sensitive to rainfall, aquifer storage capacity, recharge rate&mdash;the rate at which surface water trickles down to refill a groundwater aquifer&mdash;and human use. Trends presented here show long-term changes in groundwater, but have not been corrected for the above factors. Thus, any significant trends are not necessarily due to human use alone. However, information on long-term trends can be useful for prompting further research and informing decision-making. 
...
-  [Provincial Groundwater Observation Well Network - Groundwater Levels Data](https://catalogue.data.gov.bc.ca/dataset/57c55f10-cf8e-40bb-aae0-2eff311f1685)

- [Groundwater well metadata, such as location, depth, and aquifer type were obtained from the Ground Water Wells (Spatial View) Dataset](https://catalogue.data.gov.bc.ca/dataset/e4731a85-ffca-4112-8caf-cb0a96905778)

- [Natural Resource (NR) Regions](https://catalogue.data.gov.bc.ca/dataset/dfc492c0-69c5-4c20-a6de-2c9bc999301f), accessed with the ['bcmaps' R package](https://cran.r-project.org/web/packages/bcmaps/index.html)

----

Published and Available Online at Environmental Reporting BC (`r updateDate`): <http://www.env.gov.bc.ca/soe/indicators/water/groundwater-levels.html>

Email correspondence to: envreportbc@gov.bc.ca

*Suggested Citation*:  
Environmental Reporting BC. 2024.  Long-term Trends in Groundwater Levels in B.C. State of Environment Reporting, Ministry of Environment and Climate Change Strategy, British Columbia, Canada.
...
Trend analysis was conducted using groundwater observation wells that had ten or more years of data, fewer
...
Trend analyses were calculated both annually and monthly. For each month that had more than one observation, 
the median was taken. Annual mean groundwater levels were then calculated for each year that contained values for all twelve months.
After the time series were constructed, each was
...
(Harvey, 1990). Fixed-interval smoothing
(Durbin & Koopman, 2012) was then applied to the fitted model to generate
...
artificially increase the probability of detecting a significant trend (Yue *et al*. 2002). 
In this method, the slope of each time series
is estimated using the Theil-Sen approach (Sen, 1968). If the
...
the R package ‘zyp’ (Bronaugh *et al*, 2009).
...
- Bronaugh, D., Werner, A., & Bronaugh, M. D. (2009). Package ‘zyp’. CRAN Repository.

- Durbin, J., & Koopman, S. J. (2012). Time series analysis by state space methods (Vol. 38). OUP Oxford.

- [Gurdak, J.J., R.T. Hanson, and T.R. Green. 2009. Effects of Climate Variability and Change on Groundwater Resources of the United States. United States Geological Survey. Fact Sheet 2009–3074. (PDF)](https://pubs.usgs.gov/fs/2009/3074/pdf/FS09-3074.pdf)
...
results_viz %>%
  ...
  select(`Well Number` = Well_Num,
         ...
         `Monitoring Length (Years)` = nYears,
